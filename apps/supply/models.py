...
class Supply(BaseModel):
    ...
    def clean(self):
        if self.received_at and self.sent_at and self.sent_at >= self.received_at:
            raise ValidationError(_('"Дата отправки" не может быть раньше и равной "Дате получения"'))

    @property
    @admin.display(description=_('Стоимость партии'))
    def batch_cost(self) -> Decimal:
        total = self.products_through.aggregate(
            total=Sum(F('purchase_price') * F('qty'))
        )['total']
