...
class ProductFiltersBuilder:
    ...
    def _build_specification_filters(self) -> List[Dict]:
        ...
        for spec in specifications:
            ...
            value_exists = any(v["id"] == spec_value_id for v in spec_dict[spec_name_id]["values"])

            if not value_exists:
                spec_dict[spec_name_id]["values"].append(
                    {"id": spec_value_id, "value": spec["specification_value__value"]}
                )
    ...
    def _build_price_range(self) -> Dict[str, Optional[Decimal]]:
        ...
        """
        price_agg = self.queryset.aggregate(min_price=Min("price"), max_price=Max("price"))
