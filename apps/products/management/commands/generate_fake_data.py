...
class Command(BaseCommand):
    ...
    def _create_categories(self, count):
        ...
        for i in range(min(count, len(category_names))):
            ...
            if not Category.objects.filter(name=name).exists():
                category = Category.objects.create(name=name, position=i * 10)
                    ...
                    for j, sub_name in enumerate(subcategories_data[parent.name]):
                        if not Category.objects.filter(name=sub_name, parent=parent).exists():
                            subcategory = Category.objects.create(
                                name=sub_name, parent=parent, position=j * 10
                            )
        ...
        for i in range(count):
            ...
            if not Brand.objects.filter(name=name).exists():
                brand = Brand.objects.create(
                    name=name, description=f"Описание бренда {name}. Качественная женская одежда."
                )
                brands.append(brand)
    ...
    def _create_specifications(self):
        ...
        specs_data = {
            "Размер": {"values": ["XS", "S", "M", "L", "XL", "XXL"], "unit": None},
            "Цвет": {
                "values": [
                    "Черный",
                    "Белый",
            ...
            "Длина": {"values": ["Короткая", "Средняя", "Длинная"], "unit": None},
            "Рукав": {"values": ["Без рукавов", "Короткий", "Длинный"], "unit": None},
        }
        ...
        for spec_name, data in specs_data.items():
            spec_name_obj, created = SpecificationName.objects.get_or_create(
                name=spec_name, defaults={"position": position, "unit_measurement": data["unit"]}
            )
            position += 10
            ...
            for value in data["values"]:
                spec_value, created = SpecificationValue.objects.get_or_create(
                    specification_name=spec_name_obj, value=value
                )
                values.append(spec_value)

            specifications[spec_name] = {"name": spec_name_obj, "values": values}
    ...
    def _create_products(self, count, categories, brands, specifications):
        ...
        for i in range(count):
            ...
            product = Product.objects.create(
                ...
                f"Качественная женская одежда из натуральных материалов. "
                f"Подходит для повседневной носки.",
