...
def product_list_view(request):
    ...
    if selected_specifications:
        ...
        if spec_ids:
            ...
            spec_values = SpecificationValue.objects.filter(id__in=spec_ids).select_related('specification_name')
            ...
            # Применяем фильтрацию: товар должен иметь значение из каждой группы (AND между группами)
